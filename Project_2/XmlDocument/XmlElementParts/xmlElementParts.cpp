...
std::string XmlParts::show()
{
  ...
  for(size_t i=1; i<toks.size(); i++)
	  if (toks[i] != "\n")
	  {
	  if (toks[i] =="<"||toks[i]==">"||toks[i]=="/"||toks[i]=="?"||toks[i]=="!"||toks[i-1]=="!"||toks[i]=="="||toks[i]=="xml"||toks[i-1]=="="||toks[i-1]=="<"||toks[i-1]=="/" )
		temp.append(toks[i]);
	  else
...
//--------<parser xml file to string>----------------------
std::string XmlParts::fileparser(char argv[])
{
	std::cout << "\n  Processing file " << argv;
	std::cout << "\n  " << std::string(16 + strlen(argv), '-') ;
	std::string xml;
	try
	{
		Toker toker(argv);
...
int main(int argc, char* argv[])
{
  ...
  for(int i=1; i<argc; ++i)
  {
	  ...
	  toker.setMode(Toker::xml);
	  XmlParts parts(&toker);
	  std::cout << parts.fileparser(argv[i]) << std::endl;
  }
