...
void Display::ShowDirectory(const DataStore& ds)
{
	...
	{ 
		filestring_ += "The file name is:" + fs.first +"\n";
		std::cout << "The file name is:" << fs.first << std::endl;
		for (auto& y : fs.second)
		{
			filestring_ += "The file path is:" + *y + "\n";
			std::cout << "The file path is:" << *y << std::endl;
		}
		std::cout << std::endl;
	}
...
void Display::ShowMatchFile(const DataStore& ds, Text text_)
{
	...
	if (text_.size() != 0)
	{
		...
		{
			filestring_ += "There is no file contains the searching text.\n";
...
std::string Display::GetFileString()
{
	return filestring_;
}
...
int main(int argc, char *argv[])
{
	...
	myFilemgr.search(myDatastore.root,1,"");
