...
void Cataloger::NewCommandParser(std::string path, std::vector<std::string> patterns)
{
	path_ = path;
	for (auto patt:patterns)
	{
		int pospatt = patt.find(".");
		if (pospatt != -1)
		{
			patterns_.push_back(patt); //push the pattern into patterns
		}
	}

}
void  Cataloger::NewCommandParserXML(std::string path, std::vector<std::string> patterns)
{
	path_ = path;
	for (auto patt : patterns)
	{
...
void Cataloger::NewCommandParser2(std::string path, std::vector<std::string> patterns,std::string text)
{
	path_ = path;
	for (auto patt : patterns)
	{
	...
	int optf = text.find("/f");
	if (optf != -1)
	{
		options_.push_back(text);
		text_ = text.substr(optf + 2, text.length()); //get the searching text
	}
}
...
void Cataloger::start(sPtr& root_,int a)
{
	DataStore ds;
	Display myDisplay;
	FileMgr fm(path_, ds);
	...
	if (options_.size() != 0)
	{
		for (auto& iter = options_.begin(); iter != options_.end(); iter++)
		{
			...
			{
				fm.setRecursive();
				fm.search(root_,a,text_);
				myDisplay.ShowDirectory(ds);// ShowSummary(ds);
				replyfilestring_ = myDisplay.GetFileString();
...
std::string Cataloger::GetReturnFileString()
{
	return replyfilestring_;
}
